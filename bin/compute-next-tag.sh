...
#
# Tags look like `v<Container Socket Proxy version>-<release>`:
#
# - if defaults/main.yml points at a Container Socket Proxy version that has
#   never been released, the release counter restarts at 0 (`v0.5.1-0`)
# - otherwise the counter is incremented (`v0.5.1-1`), but only if something
...
# Anchored at the start of the line, so that this reads the leaf variable that
# Renovate rewrites, and not any of the variables derived from it
# (`container_socket_proxy_container_image_tag` and friends), whose values are
# Jinja expressions rather than versions.
version="$(sed -nE 's|^container_socket_proxy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Container Socket Proxy version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.5.0`),
